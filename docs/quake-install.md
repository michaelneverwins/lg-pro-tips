...
```
This program is actually pretty neat, in that it will create `.deb` packages for the game assets, and install them to your system in locations used by the Quakespasm source port, which is then marked as a dependency and also installed automatically. At the time of writing, it will even download the _Quake_ and mission pack soundtracks in OGG format so that Quakespasm can play the music.

But wait... isn't the soundtrack included in GOG's installer? Well, yes. It's not in OGG format, but we can convert it to that format ourselves, which is what this guide will demonstrate. Simply downloading the soundtrack is the easy solution, and probably reliable enough (assuming the site from which `game-data-packager` downloads it is allowed to host it). However, what if that site _isn't_ reliable and goes down? Or what if we just don't want to rely on `game-data-packager` to compile the required assets because we don't want them installed as system packages owned by `root`? Or what if we're using a distribution which doesn't have `game-data-packager` (but does happen to have all of the other programs to be used in the steps below)? Then we can do this:
