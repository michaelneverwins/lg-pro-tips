...
Using a Linux-native DOSBox isn't strictly necessary, as DOSBox-powered games packaged only for Windows will often run with Wine or Proton. This is sometimes the "easiest" option, particularly for running Steam's ostensibly Windows-only DOS games, many of which will "just work" out of the box on Linux if you have Proton enabled. However, using a Linux-to-Windows compatibility layer to run the Windows version of a Linux-compatible DOS emulator is probably not ideal for any game.
...
* There's another Steam Play compatibility tool from the creator of Boxtron and Roberta, called Luxtorpeda ([`https://github.com/luxtorpeda-dev/luxtorpeda`](https://github.com/luxtorpeda-dev/luxtorpeda)), for automatically running Steam games with their respective Linux-native engines.
  * Supported games: [`https://luxtorpeda-dev.github.io/packages.html`](https://luxtorpeda-dev.github.io/packages.html)
* GameDataPackager ([`https://wiki.debian.org/Games/GameDataPackager`](https://wiki.debian.org/Games/GameDataPackager)) is a command-line tool which, if provided data files of certain non-free games, packages and installs them along with the dependencies (e.g. source ports) required to play them on Linux.
  * Supported games: [`https://salsa.debian.org/games-team/game-data-packager/tree/master/data`](https://salsa.debian.org/games-team/game-data-packager/tree/master/data)
