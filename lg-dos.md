...
If you've manually installed DOSBox Staging or some other DOSBox fork to a directory which is not in your `PATH`, using it with Boxtron will require editing the `cmd = ...` line in the `[dosbox]` section of `~/.config/boxtron.conf`. For more on configuration of Boxtron, see the wiki (`https://github.com/dreamer/boxtron/wiki/Configuration`)[https://github.com/dreamer/boxtron/wiki/Configuration].
